...
static void print_binary(byte *bin, int length) {
  int i;
  DEBUG("<<");
  for (i = 0; i < length - 1; i++) {
    DEBUG("%i, ", (int)bin[i]);
  }
  DEBUG("%i", (int)bin[length - 1]);
  DEBUG(">>\n");
}
...
int ore_blk_compare(int* result_p, ore_blk_ciphertext ctxt1, ore_blk_ciphertext ctxt2) {
  ...
  bool is_equal = true;
  for (int i = 0; i < nblocks; i++) {
    DEBUG("Checking block %d at offset %lu", i, offset_left);
  ...
  print_binary(ctxt1->comp_left, 32);
  DEBUG("In compare, offset_left = %lu, AES_KEY_BYTES = %d, ceil = %d, index = %llu\n", offset_left, AES_KEY_BYTES, CEIL(block_len, 8), index);
...
int init_ore_blk_ciphertext(ore_blk_ciphertext ctxt, ore_blk_params params) {
  ...
  ctxt->left_size = _ore_blk_ciphertext_len_left(params);
  ctxt->comp_left = malloc(ctxt->left_size);
  if (ctxt->comp_left == NULL) {
    return ERROR_MEMORY_ALLOCATION;
  }

  ctxt->right_size = _ore_blk_ciphertext_len_right(params);
  ctxt->comp_right = malloc(ctxt->right_size);
